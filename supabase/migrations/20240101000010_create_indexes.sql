-- Migration 0010: Cria índices secundários para queries frequentes.
-- Cobre lookup por `user_id`, `instagram_user_id`, janela 24h em `contacts`,
-- agendamento de broadcasts, runs ativos e timeline de mensagens. Índices
-- parciais (com WHERE) reduzem custo de manutenção em colunas opcionais.

-- accounts
CREATE INDEX IF NOT EXISTS idx_accounts_user_id ON accounts(user_id);
CREATE INDEX IF NOT EXISTS idx_accounts_instagram_user_id ON accounts(instagram_user_id);

-- contacts
CREATE INDEX IF NOT EXISTS idx_contacts_account_id ON contacts(account_id);
CREATE INDEX IF NOT EXISTS idx_contacts_window_expires ON contacts(account_id, window_expires_at)
...
CREATE INDEX IF NOT EXISTS idx_automations_status ON automations(account_id, status);

-- steps
CREATE INDEX IF NOT EXISTS idx_steps_automation_id ON steps(automation_id);
...
CREATE INDEX IF NOT EXISTS idx_broadcasts_status ON broadcasts(account_id, status);
...
CREATE INDEX IF NOT EXISTS idx_messages_account ON messages(account_id, sent_at DESC);
...
-- webhook_events
CREATE INDEX IF NOT EXISTS idx_webhook_unprocessed ON webhook_events(received_at)
